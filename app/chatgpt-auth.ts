...
export async function getChatGPTUser(): Promise<ChatGPTUser | null> {
  ...
  if (!userId || !email) {
    const accessEmail = requestHeaders
      ...
      ?.trim()
      .toLowerCase();
    const accessJwt = requestHeaders.get(CLOUDFLARE_ACCESS_JWT_HEADER);
    if (!accessEmail || !accessJwt) return null;

    return {
      userId: `cloudflare-access:${accessEmail}`,
...
export function authenticatedUserSignOutPath(user: ChatGPTUser): string {
